class UserSeeder
  SEEDS_FILENAME = 'db/seeds/users.csv'.freeze
    ...
    if File.exist?(SEEDS_FILENAME)
      ...
      CSV.foreach(SEEDS_FILENAME, headers: true) do |row|
        ...
        if UserCredential.exists?(email: row['email_address']) ||
             User.exists?(email_address: row['email_address'])
          puts "Skipping [#{row_index}:#{row['email_address']}] because that email address already exists"
          next
        end
      ...
      puts "Create the file [#{SEEDS_FILENAME}] to seed users"
    end
  end
  ...
  def seed_row(row_index, row)
    ...
    unless user_credential.valid? && user.valid?
      puts "Skipping [#{row_index}:#{row['email_address']}]: the row is not valid:"
